...
class TranscodingJob(Job):
    ...
    def run(self):
        ...
        if self.jobCodec == "x264" and not path.exists(f"{self.dashboard.fileSystem.basePath}/processed/{self.jobPath}/episode_x264.mp4"):
            args = ["../scripts/ffmpeg-x264-medium.sh", self.dashboard.fileSystem.getFile(self.jobSrcPath), f"{self.dashboard.fileSystem.mountPath}/processed/{self.jobPath}"]
            ...
            args = ["../scripts/ffmpeg-vp9.sh", self.dashboard.fileSystem.getFile(self.jobSrcPath), f"{self.dashboard.fileSystem.mountPath}/processed/{self.jobPath}"]
        ...
        if self.jobCodec == "x264" and not path.exists(f"{self.dashboard.fileSystem.basePath}/processed/{self.jobPath}/hls/x264/master.m3u8"):
            ...
            audioStreams = subprocess.getoutput(f'ffprobe -v error -select_streams a -show_entries stream=index -of csv=p=0 "{self.dashboard.fileSystem.getFile(self.jobSrcPath)}" | wc -w')
            script = "../scripts/ffmpeg-x264-hls.sh" if int(audioStreams) <= 1 else "../scripts/ffmpeg-x264-hls-dub.sh"
            args = [script, self.dashboard.fileSystem.getFile(f"processed/{self.jobPath}/episode_x264.mp4"), f"{self.dashboard.fileSystem.basePath}/processed/{self.jobPath}"]
            self.jobSubprocess = subprocess.Popen(args, stdin=DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
